function [ manipulatorInfo ] = featureManipulator_DB2dPCA(manipulatorInfo)
%Reed Gurchiek, 2018
%   description
%
%----------------------------------INPUTS----------------------------------
...
if strcmpi(manipulatorInfo.action,'train')
    ...
    [pc,pcvar] = dpca(features(dbKeepIndices,:)');
    
    % if no PCA keep criteria
    if ~isfield(manipulatorInfo,'dpcaKeepCriteria')
        
        % get criteria
        dpcaKeepCriteria = questdlg('Discrete number of features or component variance based criteria for keeping features?','Keep Criteria','Number of Features','Minimum PC Variance','Minimum PC Variance');
        
        % display pcs and variance and instruct user
        instruction = sprintf('Observe the list of principal components and determine the threshold %s',dpcaKeepCriteria);
        questdlg(instruction,'Threshold Determination','OK','OK');
        ...
        % confirm or alter
        if strcmp(dpcaKeepCriteria,'Number of Features') 
            pcaThreshold = ipc;
            manipulatorInfo.dpcaKeepCriteria = 'nFeatures';
        else
            pcaThreshold = pcvar(ipc); 
            manipulatorInfo.dpcaKeepCriteria = 'componentVariance';
        end
        prompt = sprintf('Confirm or alter %s threshold = %4.2f',dpcaKeepCriteria,pcaThreshold);
    ...
    if strcmp(manipulatorInfo.dpcaKeepCriteria,'nFeatures')
